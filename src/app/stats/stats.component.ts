...
export class StatsComponent implements OnInit {
  ...
  initializeVisitsByMonthChart() {
    ...
    const visitsByMonthLabels = [];
    const visitsByMonthData = [];
    const visitsByMonthBgColors = [];
  ...
  initializeVisitsBySocialSecurityChart() {
    ...
    const visitsBySocialSecurityLabels = [];
    const visitsBySocialSecurityData = [];
    const visitsBySocialSecurityBgColors = [];
    for (let i = this.data[4].length - 1; i >= 0; i--) {
      const socialSecurity = this.catalogsService.getSocialSecurity(this.data[4][i].socialSecurity);
      if (socialSecurity) {
